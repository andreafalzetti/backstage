  ...
  return createTemplateAction<{ files: FilesToRename }>({
    ...
    async handler(ctx) {
      ...
      for (const file of ctx.input.files) {
        ...
        const sourceFilepath = `${ctx.workspacePath}/${file.from}`;
        const destFilepath = `${ctx.workspacePath}/${file.to}`;

        if (sourceFilepath.includes('..') || destFilepath.includes('..')) {
          throw new InputError(
            'You cannot rename files outside of your workspace',
          );
        }
